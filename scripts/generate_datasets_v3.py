"""
KitchenGuard CSM - Enhanced Dataset Generator v3 (FIXED PATHS)
...
# Setup paths yang benar dari mana saja
...
def generate_enhanced_waste_dataset(samples_per_class=500):
    ...
    for category, content in categories.items():
        print(f"  Generating {category} samples ({samples_per_class} each)...")
...
if __name__ == "__main__":
    print("="*70)
    print("🍽️ KitchenGuard CSM - Enhanced Dataset Generator v3 (FIXED)")
    print("="*70)
    ...
    skin_file = os.path.join(DATA_DIR, "skin_detection_dataset.csv")
    skin_df.to_csv(skin_file, index=False)
    print(f"✓ Saved: {skin_file}")
    print(f"  Samples: {len(skin_df)} (ENHANCED!)")
    print(f"  Categories:")
    ...
    waste_df = generate_enhanced_waste_dataset(500)
    waste_file = os.path.join(DATA_DIR, "waste_quality_dataset_expanded.csv")
    waste_df.to_csv(waste_file, index=False)
    print(f"✓ Saved: {waste_file}")
    print(f"  Samples: {len(waste_df)} (ENHANCED!)")
    ...
    print("="*70)
    print(f"\n📁 Output Files:")
    print(f"  1. 🧴 skin_detection_dataset.csv - {len(skin_df)} samples")
    print(f"  2. ♻️  waste_quality_dataset_expanded.csv - {len(waste_df)} samples")
    print("\n🚀 Next steps:")
    print("   cd src")
